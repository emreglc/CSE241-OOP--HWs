...
void Tetromino::rotate(string dir)
{
	...
	if(dir == "right")
	{
		...
		{
			for (size_t col = 0; col < temp[row].size()/2; col++)
			{
	...
	if(dir == "left")
	{
		...
		for (size_t row = 0; row < this->getTetro().size(); row++)
		{
			for (size_t col = 0; col < this->getTetro()[row].size()/2; col++)
			{
...
void Tetromino::mostFit(vector<Tetromino> &tetros)
{
	...
	for(int i = 0; i < tetros.size() -1; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			if(tetros[i].canFit(tetros[i+1]) == true)
				break;
			else
